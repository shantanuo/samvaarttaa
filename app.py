...
#EMAIL_RECEIVER = st.secrets.get("email_receiver", "shantanu.oak@gmail.com")
    ...
    with col1:
        if st.form_submit_button("📋 Paste"):
            try:
                ...
                st.error("Failed to paste text. Please ensure clipboard access is enabled or paste manually.")
    with col2:
    ...
    if not input_text.strip():
        ...
        try:
            with st.spinner("Generating Sanskrit translation... Please wait."):
                ...
                # Display output in command-prompt-like box
                st.markdown("### Output")
                st.write(f"```\n{response}\n```")
